...
one dated Snapshot per tick. WS subscription for real-time mark-price and
funding lands here later (M1 phase B) once the poll cadence is validated.
...
class LiveWatcher:
    ...
    async def snapshot(self) -> Snapshot:
        ...
        try:
            ...
            self.watchdog.mark_ws_tick(now=now_mono)  # polling counts as "fresh" for M1
        except Exception:
        ...
        return Snapshot(
            ...
            wsteth_price_usd=meta.mark_price,
            ...
            borrow_apr=0.0,  # TODO M1-B: read from Aave reserve data
            gas_eth=gas,
