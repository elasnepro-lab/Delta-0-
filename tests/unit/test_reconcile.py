...
@pytest.mark.asyncio
async def test_first_boot_no_anchor(store: StateStore, config: Config) -> None:
    report = await reconcile_at_boot(store, _snap(), config)
...
@pytest.mark.asyncio
async def test_small_anchor_drift_no_warning(store: StateStore, config: Config) -> None:
...
@pytest.mark.asyncio
async def test_large_anchor_drift_warns(store: StateStore, config: Config) -> None:
    await store.kv_set("anchor_price", "2500.0")
...
@pytest.mark.asyncio
async def test_debt_drift_warns(store: StateStore, config: Config) -> None:
...
@pytest.mark.asyncio
async def test_low_hf_critical(store: StateStore, config: Config) -> None:
...
@pytest.mark.asyncio
async def test_debt_last_written_after_reconcile(store: StateStore, config: Config) -> None:
    await reconcile_at_boot(store, _snap(), config)
